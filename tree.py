...
def reConstructBinaryTree(pre, tin):
    if not pre:
        ...
        t = TreeNode(pre[0])
        subPreLeft = pre[1 : tin.index(pre[0])+1]
        subTinLeft = tin[0 : tin.index(pre[0])]
        t.left = reConstructBinaryTree(subPreLeft, subTinLeft)
        subPreRight = pre[tin.index(pre[0])+1 :]
        subTinRight = tin[tin.index(pre[0])+1 :]
        t.right = reConstructBinaryTree(subPreRight, subTinRight)
    return t
